...
export class LoginService {
  ...
  loginUser(creds: { user: string; password: string; rememberMe: boolean }): Observable<boolean> {
    ...
    return this.http.post<any>(`/api/admin/auth`, body, { headers, withCredentials: true }).pipe(
      tap((authRes: any) => {
        ...
        this.http.get<UserDataResponse>(`/api/token/access`, { withCredentials: true }).pipe(
  ...
  refreshToken(): Observable<boolean> {
    ...
    return this.http.get<any>(`/api/token/refresh`, { withCredentials: true }).pipe(
      tap(() => {
